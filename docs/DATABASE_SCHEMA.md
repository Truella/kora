...
Migration `20260924120000_circle_invites.sql` (directed phone invites): fixes `?by=` to carry the inviter's **profile id** (`is_valid_inviter` now checks active membership by `user_id` — the old member-row id failed the `invited_by → profiles(id)` FK with 23503, so no attributed link invite ever saved; prod had 1 request, 0 attributed). New `circle_invites(group_id, inviter_user_id → profiles, invitee_phone E.164, status pending/accepted/declined)` with a partial unique `(group_id, invitee_phone)` where pending: active members insert their own invites (ownership half: `inviter_user_id = auth.uid()`), sender + invitee (own-verified-phone match, no directory) can select, sender can delete pending, no client updates. `reject_invite_for_member` trigger refuses numbers already active in the circle (`already_member`/P0001). `my_pending_invites()` (security definer) feeds `/home`; `accept_circle_invite()` files the same `join_requests` row the link flow files (inviter profile id, NULL if they left; clears own rejected row; idempotent `accepted`/`already_pending`/`already_member`) and `decline_circle_invite()` consumes the nudge. Accept never admits directly — membership still flips only via `tally_join_votes`.
