...
- **Auth:** Supabase Auth, phone-primary — phone number first → SMS OTP → new users onboard (name + country), existing users straight to dashboard, `phone_verified = true` on verify. Email path sends a **sign-in link** (magic link, exchanged at `/auth/callback` — no code to type): account works in the PWA but the user must add + verify a phone number before anything USSD-related. Phone is the anchor identity (USSD has no email), stored E.164. Skip formal KYC entirely for MVP
- **Icons:** Hugeicons
- **Animation:** Motion
